...
namespace Microsoft.CognitiveSearch.WebApiSkills
{
    public static class JfkWebApiSkills
    {
        ...
        [FunctionName("identify-sdgs")]
        public static IActionResult RunSDGIdentifier([HttpTrigger(AuthorizationLevel.Function, "post", Route = null)] HttpRequest req, TraceWriter log, ExecutionContext executionContext)
        {
            string skillName = executionContext.FunctionName;
            IEnumerable<WebApiRequestRecord> requestRecords = WebApiSkillHelpers.GetRequestRecords(req);
